...
export function calculateShipping(weight, country, orderTotal) {
  ...
  var is_domestic = country == "US";

  if (is_domestic && orderTotal > 50) return 0;

  if (!is_domestic && orderTotal > 100) return 0;


  if (weight <= 1) {
    if (is_domestic) return 5;
    return 15;
  } else if (weight <= 5) {
    if (is_domestic) return 10;
    return 25;
  } else {
    if (is_domestic) return 15;
    return 40;
  }
